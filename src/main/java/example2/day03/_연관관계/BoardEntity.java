...
/*
    [ 영속성 ]  자바 데이터(객체) 와 데이터베이스 데이터를 매핑(연결)
        - 즉] 영속 중 이라면 자바 객체 수정시 DB 데이터 수정.
        - 즉] 영속 중 아니라면 자바 객체 수정해도 DB 데이터 그대로.
    PK - FK 제약조건 옵션
    [ cascade ]
        ...
        cascade = CascadeType.DETACH : 부모가 영속 해제되면 자식도 같이 해제된다.

    [ fetch ] 부모와(PK) 자식(FK) 관계에서 엔티티를 조회하는 방법
        fetch = FetchType.EAGER : 해당 엔티티를 조회하면 참조 엔티티도 즉시 조회한다.
            - 특징 : 기본값 , 초기 로딩 느림 , 불필요한 엔티티 정보가 있을 경우 기능 저하
        fetch = FetchType.LAZY : 해당 엔티티를 조회하면 참조 엔티티를 조회하지 않는다.
            - 특징 : 초기 로딩 빠름 , 사용할 엔티티 정보를 적절하ㅏ게 사용하면 성능 최적화 가능
            - 해당 엔티티.getXXX() 하는 순간 그때 참조 엔티티 조회

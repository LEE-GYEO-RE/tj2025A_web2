...
# 뷰 사용시 주의할점 : 특정한 경우가 아니면 select 쿼리만 가능. 뷰에서는 update , delete , insert 조건 사용
# 뷰는 주로 읽기모드 사용
# 수정이 불가능 뷰 : JOIN(집계) , GROUP BY , 함수 , 계산식(집계/통계) 포함하면 읽기모드 만 가능
# 수정이 가능한 뷰 : 단일 테이블 기반 ( JOIN,통계,집계 등 없는 테이블 )

# [2] 조건 뷰 생성 , 만약에 회원 100만명 이상일때 조회 속도 평가하면 몇배의 성능 차이 있을 수 있음.
# 1. vip 뷰 만들기
...
select * from member where grade = 'VIP'; -- 원본 조회 : 결과는 같지만 조회 속도에서는 느림.
...
# [4] 뷰의 데이터를 수정하는 조건 : 단일 테이블의 원본 필드이면 가능 , 집계/통계/그룹/계산 등등 필드에 대해서는 불가능.

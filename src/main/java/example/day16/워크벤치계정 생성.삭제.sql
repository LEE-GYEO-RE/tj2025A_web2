...
# [1] 계정 만들기 : root(최고관리자) 계정으로 가능
# 데이터베이스 설치하면 기본적으로 root 계정이 존재 < 강의기준 : root 사용중 >
...
# [2] (워크벤치) 다른 계정으로 접속하기 : 
# 1. 워크벤치 홈 메뉴에서 ( Database -> Connect to database ) --
# 2. username : 계정명 , password : 비밀번호 , [ok] 
# 다른 계정으로 접속 후 : show databases;

# [3] 계정의 권한 부여 ( GRANT ) , all privileges : springweb2에 대한 모든 권한 허용
# grant all privileges on 데이터베이스명.테이블/뷰명 to "계정명"@"도메인";
grant all privileges on springweb2.* to "dev1"@"localhost"; # 확인 : dev1 계정에서 접속후 확인
grant select on springweb2.student to 'dev2'@'%';			
# 특정한 테이블만 조회 권한 부여  ( student 테이블에서 select 조회만)
-- use springweb2; 			됨
-- select * from student;	됨
-- delete from student;		안됨
...
# revoke select on 데이터베이스명.테이블/뷰명 from '계정명'@'도메인';
...
# 인텔리제이 ,  VSCODE = IDE 통합개발 환경 : 코드를 편하게 사용할 수 있게 도와주는 소프트웨어

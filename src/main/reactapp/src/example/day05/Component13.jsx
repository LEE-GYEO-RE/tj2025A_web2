...
export default function Component13 (props){
    ...
    // const { 상태변수명 } = useSelector( (state) => state.store에 등록된 상태명 )
    const { isAuthenticated } = useSelector( (state) => state.user );
    console.log( isAuthenticated );

    // [2] 액션 이용한 전역변수 상태 변경하기
    const dispatch = useDispatch();

    // [3] 로그인 처리
    const loginHandle = () => {    // 로그인 axios 했다 가정하에
        dispatch( login() ); // dispatch 이용한 login 액션 요청. dispatch( login리듀서 )
    }

    // [4] 로그아웃 처리
    const logoutHandle = () => {   // 로그아웃 axios 했다 가정하에
        dispatch( logout() ); // dispatch 이용한 logout 액션 요청 dispatch( logout리듀서 )
    }

...
# 주의할 점 : 첫번째 인덱스에 대해서는 단일 사용 가능 , 두번째 인덱스부터는 단일 사용 불가능
...
# [7] JOIN : PK - FK 가 아니더라도 조건 가능 , PK는 unique 제공
# FK에 인덱스 추가
create index idx_dept on employee( dept_id );

# FK에 인덱스 추가 후 JOIN 하면 조인 속도가 증가한다.
select * from employee e inner join department d on e.dept_id = d.dept_id;

# [8] 문자열 검색 : 자연어( 사람이 사용하는 언어 ) VS 기계어( 컴퓨터 2진수 )
# type이 text , longtext , char , varchar 만 가능
create fulltext index idx_name_full on employee(name);
explain analyze select * from employee where match(name) against('유재석'); -- name = '유재석' 이랑 조회속도가 다름  
